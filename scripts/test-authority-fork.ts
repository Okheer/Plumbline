...
await send(lifecycle(authority,state.resource,agent,false));
assert.equal(await client.readContract({address:authority,abi:registryABI,functionName:'hasRoles',args:[state.resource,AGENT_ACTIVE,agent]}),false);
await send(scopeResolver(resolver,full,agent,owner),agent);
await send(mandateRecords(resolver,full,1n,['0x0000000000000000000000000000000000000011']),owner);
assert.equal(await client.readContract({address:resolver,abi:resolverABI,functionName:'text',args:[namehash(full),'plumbline.mandate.maxSlippageBps']}),'50');
await assert.rejects(client.simulateContract({account:agent,address:resolver,abi:resolverABI,functionName:'setText',args:[namehash(full),'plumbline.mandate.maxSlippageBps','9999']}));
